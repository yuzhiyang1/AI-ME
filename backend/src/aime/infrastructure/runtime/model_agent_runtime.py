        ...
        for call in calls:
            ...
            if finished_invocation_id is not None:
                ...
                if managed is not None and call.name != "skill_read":
                    # 压缩模型可见结果，但审计账本仍保留界面所需的前后差异。
                    projected = await managed.project_result(result.model_output)
                    if "file_change" in result.output:
                        projected = {**projected, "file_change": result.output["file_change"]}
                    result = ToolExecutionResult(
                        projected,
                        result.is_error,
                    )
