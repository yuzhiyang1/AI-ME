...
| `AI_ME_FEISHU_DRAFT_TIMEOUT_MS` | 后台回复草稿的模型生成上限，默认 30000ms；Webhook 不等待模型，超时后保留保守草稿并记录错误 |
...
- Webhook 处理不继承客户端断连，并先落 `pending_ai_model` 审批后立即响应；DeepSeek 在后台原子升级仍处于待处理状态的草稿，不能覆盖人工编辑或已批准内容。
- 模型超时不能阻止收件箱和审批落库；明确失败的事件允许飞书重试重新消费。
